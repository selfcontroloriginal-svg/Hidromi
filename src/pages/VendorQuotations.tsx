...
import { ArrowLeft, Plus, Edit, Trash2, Send, Share2, Download } from 'lucide-react';
...
function VendorQuotations() {
  ...
  const quotations = useQuotationStore(state => state.getQuotationsByVendor(profile?.id || ''));
  ...
  const handleStatusChange = async (id: string, status: 'sent') => {
    try {
